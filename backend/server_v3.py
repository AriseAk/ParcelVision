...
# Gamma corrects depth-model non-linearity. Tune 0.8–1.1 if dimensions
# are consistently over/under-estimated.
GAMMA = 0.9
...
def depth_to_metric(depth_values: np.ndarray,
    ...
    gamma        : float      – non-linearity correction (0.8–1.1)
...
def detect():
    try:
        ...
        print(f"📷 fx={req_fx:.1f}  scale={scale}  Z_cal={scale*req_fx:.3f}m  motion={motion:.2f}" if scale else f"📷 fx={req_fx:.1f}  scale=None  motion={motion:.2f}")
...
# /scan_frame
#
# Uses card scale + fx to compute Z_cal (calibration distance in metres).
# Every pixel's depth is derived as Z = Z_cal / d_corr (inverse disparity).
# Translation from VO is discarded — only rotation is applied to reduce
# multi-view noise without introducing scale drift.
# ─────────────────────────────────────────────────────────────────────────────
...
def scan_frame():
    try:
        ...
        # Fall back to last known detection if YOLO misses on this frame
        if not bboxes:
            if last_detections:
        ...
        z_ok     = (Z_metric > 0.02) & (Z_metric < 10.0)
        print(f"   Z range: {Z_metric.min():.3f}–{Z_metric.max():.3f}m  valid={z_ok.sum()}/{len(z_ok)}")
        ...
        pts_world = (current_R @ pts_cam.T).T  # rotation only, no translation
...
def compute_dimensions():
    try:
        ...
        raw_dims = sorted(
            [float(projected[:, i].max() - projected[:, i].min()) for i in range(3)],
            reverse=True
        )
...
def start_calibration():
    return jsonify({"status": "deprecated — use card calibration"})
